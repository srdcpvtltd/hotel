<div class="btn-group">
    ...
    <div class="dropdown-menu" x-placement="bottom-start">
        ...
        @can('edit-System Management')
            <a href="{{ route('price_rule.edit', $pricerule->id) }}" class=" dropdown-item"><i
                    class="cil-pencil action-btn"></i>
                {{ __('Edit') }}</a>
        @endcan
        <div class="dropdown-divider"></div>
        @can('delete-System Management')
            <a href="{{ route('price_rule.index') }}" class="dropdown-item  text-danger"
                data-toggle="tooltip" data-original-title="{{ __('Delete') }}"
                onclick="delete_record('delete-form-{{ $pricerule->id }}')"><i
                    class="cil-trash action-btn"></i>{{ __('Delete') }}</a>
            {!! Form::open(['method' => 'DELETE', 'route' => ['price_rule.destroy', $pricerule->id], 'id' => 'delete-form-' . $pricerule->id]) !!}
            {!! Form::close() !!}
        @endcan
    </div>
</div>
