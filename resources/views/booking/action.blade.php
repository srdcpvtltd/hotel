<div class="btn-group">
    ...
    <div class="dropdown-menu" x-placement="bottom-start">
        ...
        @can('edit-Booking')
            <a href="{{ route('booking.edit', $booking->id) }}" class=" dropdown-item"><i
                    class="cil-pencil action-btn"></i>
                {{ __('Edit') }}</a>
        @endcan
        <div class="dropdown-divider"></div>
        @can('delete-Booking')
            <a href="{{ route('booking.index') }}" class="dropdown-item  text-danger"
                data-toggle="tooltip" data-original-title="{{ __('Delete') }}"
                onclick="delete_record('delete-form-{{ $booking->id }}')"><i
